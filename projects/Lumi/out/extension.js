...
function activate(context) {
    ...
    // --- 2. "Lumi: 시작" 명령어 등록 ---
    context.subscriptions.push(vscode.commands.registerCommand('lumi.start', () => {
        ...
        if (!backendProcess) {
            ...
            backendProcess = (0, child_process_1.spawn)(exePath, [], { windowsHide: true });
...
function createOrShowWebviewPanel(context) {
    ...
    currentPanel = vscode.window.createWebviewPanel('lumiPanel', 'Lumi Assistant', column || vscode.ViewColumn.One, {
        enableScripts: true,
...
function getWebviewContent(context, buildPath, webview) {
    ...
    html = html.replace(/(src|href)="\.\/assets\/(.*?)"/g, `$1="${baseUri}/assets/$2"`);
